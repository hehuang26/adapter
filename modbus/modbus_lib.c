...
#ifdef WIN32
  mb_param->fd = INVALID_HANDLE_VALUE;
...
static int modbus_connect_rtu(modbus_param_t *mb_param)
{
  ...
  if (mb_param->fd != INVALID_HANDLE_VALUE && !CloseHandle(mb_param->fd)){
    printf("Can't close comm port");
    return -1;
  }
  mb_param->fd = INVALID_HANDLE_VALUE;
         
/*	mb_param->fd = CreateFileA((LPCSTR)mb_param->device,
  ...
  sprintf(portIn,mb_param->device);
  lpFileName = (LPCSTR)portIn;
  printf("lpFileName: %s\n",lpFileName);
  mb_param->fd = CreateFileA(lpFileName,
                             GENERIC_READ|GENERIC_WRITE,
                             0,
                             NULL,
                             OPEN_EXISTING,
  ...
  if (GetCommState(mb_param->fd,&config_) == 0)
	{
		printf("Get configuration port has problem.\n");
		return FALSE;   
	}
  config_.BaudRate = mb_param->baud;  // Specify buad rate of communicaiton.
  config_.StopBits = mb_param->stop_bit;  // Specify stopbit of communication.
  config_.Parity = mb_param->parity;      // Specify parity of communication.
  config_.ByteSize = mb_param->data_bit;  // Specify  byte of size of communication.
  if (SetCommState(mb_param->fd,&config_) == 0)
	{
	...
	SetCommTimeouts(mb_param->fd,&comTimeOut);
         
  if(mb_param->fd == INVALID_HANDLE_VALUE) {
    char port[64];
    strcpy(port, "\\\\.\\");
    strcat(port, mb_param->device);
    mb_param->fd = CreateFileA(port,
                               GENERIC_READ|GENERIC_WRITE,
                               0,
                               NULL,
                               OPEN_EXISTING,
  ...
  if(!SetupComm(
       mb_param->fd,    // handle of communications device
  ...
  dcb.Parity = parity;
  dcb.ByteSize = mb_param->data_bit;
  ...
  if(!SetCommTimeouts(mb_param->fd, &cto)){
    printf("SetTimeouts failed ");
    return -1;
  }      
         
  if(!SetCommState(mb_param->fd, &dcb)){
...
static int modbus_connect_tcp(modbus_param_t *mb_param)
{
  ...
  mb_param->sock = socket(AF_INET, SOCK_STREAM, 0);
  ...
  ret = connect(mb_param->sock, (struct sockaddr *)&addr,
...
{
  shutdown(mb_param->sock, SHUT_RDWR);
  closesocket(mb_param->sock);
}
...
SOCKET modbus_init_listen_tcp(modbus_param_t *mb_param)
{
  int ret;
  SOCKET new_socket;
  struct sockaddr_in addr;
  ...
  mb_param->sock = accept(new_socket, (struct sockaddr *)&addr, &addrlen);
  if (ret < 0) {
    perror("accept");
    closesocket(new_socket);
    new_socket = 0;
